...
The agent calls `audit_project` → gets a standard `Report` of findings straight from your source (plus the rendered page's `Report` under `live_audit`) → calls `explain_finding` / `generate_fix` on the finding ids it wants to fix → **applies the returned content to your source** → calls `audit_project` again to confirm the score rose. astova diagnoses and supplies the exact fix content; your agent edits the files.
