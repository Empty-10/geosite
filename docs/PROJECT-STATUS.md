...
> estimate**, and generates fixes — not just findings. Codename `astova` (placeholder).
...
pip install -e .
python -m astova_engine https://stripe.com          # human-readable report
python -m astova_engine https://stripe.com --json    # JSON
pytest                                               # offline tests

# Optional: scan the post-JavaScript DOM (flags JS-dependent content)
pip install -e ".[render]" && playwright install chromium
python -m astova_engine https://example.com --render
...
python -m astova_engine https://example.com --performance
```
...
## What's built — engine (`engine/astova_engine/`)
...
1. ~~**Connect the demo to the real engine — `/api/scan`.**~~ ✅ **Done.**
   ...
   error state). Chosen approach: **shell out** (`python -m astova_engine <url> --json`),
   isolated behind one function and gated on `ASTOVA_ENGINE_URL` so production can swap to
   ...
   The FastAPI engine service backing the `ASTOVA_ENGINE_URL` path now exists too:
   `engine/astova_engine/service.py` (`pip install -e ".[service]"`, then
   `uvicorn astova_engine.service:app --port 8000`). `POST /scan {url}` returns the same
...
- `web/app/layout.tsx` → `metadataBase` is `https://astova.example`. Set the real domain.
- `astova` is a placeholder name; the `.ai` domain isn't locked yet.
- The `/api/scan` route shells out to a local Python process by default — works in local dev
  and on a container host, **not** on Vercel serverless. For production set `ASTOVA_ENGINE_URL`
  to the deployed FastAPI engine service (`engine/astova_engine/service.py`); that service
