...
Shared limitations: no auth, no rate limit, no timeout, no logging, no caching, no scan reuse / context between tools. (The per-finding remediation loop: `explain_finding` to understand a finding, `generate_fix` to get the exact snippet to apply, then `verify_fix` to re-scan and confirm it is resolved.)
