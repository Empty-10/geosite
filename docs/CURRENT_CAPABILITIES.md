...
- Runs ~58 checks across five areas:
  ...
  - **Expert Answerability Review (`answerability_review.py` + `answerability.py`, GEO pillar):** answers "could ChatGPT/Claude/Gemini confidently quote this page?". Adds 5 new deterministic findings - `geo.answer_self_contained` (up-front answer doesn't open with an unresolved 'It/This/They'), `geo.heading_coverage` (enough H2/H3 sections for the length), `geo.table_extractability` (data tables have header rows), `geo.question_coverage` (no orphan question headings), `geo.definition_present` (a quotable 'X is a…' near the top) - on top of the existing geo answerability checks (all kept first-class, incl. `geo.frontload`). The synthesis (verdict Strong/Partial/Weak, **Review Confidence** high/medium/low with reasons, **Likely AI Quote**, a deterministic consultant summary, section breakdown, counts) is attached additively under `scorecard.reviews.answerability` via the shared **Expert Review contract** (`reviews.py`: `review_confidence` / `classify_findings` / `build_review` - reused by every future review). No new auto-fixes (answerability is editorial → AI-assisted/manual). Surfaced as an "Answerability Review" card in the web report.
  - **Expert Schema Review (`schema_review.py`, on-page pillar):** a deeper deterministic pass over the page's JSON-LD graph emitting `schema.duplicate_organization` / `duplicate_localbusiness` / `duplicate_website`, `conflicting_organization_name` / `url` / `logo`, `missing_id`, `duplicate_id_conflict`, `orphan_node`, `invalid_sameas`, `weak_sameas`, `article_missing_publisher` / `article_missing_author`, `website_missing_searchaction` (the one with a deterministic fix), `breadcrumb_disconnected`, `canonical_mismatch`, `insecure_url`, `image_missing_dimensions`, `generic_type`. Findings feed scorecard row 20; surfaced as an "Expert Schema Review" card in the web report. Wording avoids overclaiming ranking impact.
...
- `scorecard`: `{confidence:"verified", headline_score, technical_score, overlay{total,max,factors}, rows[20]{n,label,score,status,findings,impact}, categories[5], summary{verdict,...}, citation{...}, reviews{<key>:ReviewContract}, assessment{...}}`
- `scorecard.reviews`: the standard Expert Review contract per review (`answerability`, `schema`) - verdict / confidence{level,reasons} / summary / likely_ai_quote / sections / counts / related_findings (`reviews.py` helpers).
- `scorecard.assessment` (**Consultant report**, `assessment.py`): the deterministic **Implementation Programme** (phases Quick Wins / Technical Foundations / Content Improvements / Authority & Business, each with objective, effort-time, expected AI Readiness improvement, fix count, AI-agent suitability stars, manual-review requirement), the **Review Comparison** (per-review verdict/confidence/issues/recoverable-impact/maturity), **highest_roi_review**, aggregate **confidence**, and a programme-aware **verdict**. Impact from the scorecard rows, effort from `effort.py`, remediation class from `reviews.py` - no invented estimates beyond a fixed minutes-per-fix model. Reused by the live/shared/print web reports and the Markdown export.
...
- **Consultant report (live `/report`, shared `/report/[id]`, print):** all three surfaces render the same hierarchy from `scorecard.assessment` - Executive Assessment (score + band + Astova confidence) -> Overall Consultant Verdict -> Expert Reviews (comparison + Highest-ROI + the Answerability/Schema cards) -> Implementation Programme (the four phases) -> Verification -> a collapsed **Detailed findings & evidence** appendix (scorecard, all checks, pillars, performance, citation, bot view). Progressive disclosure; nothing removed, evidence moved below decisions. Components in `report/ConsultantReport.tsx`.
