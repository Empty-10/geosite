...
/**
 * A plain-text / Markdown checklist of everything the page is missing or needs, for copy or
 * download. Purely deterministic — it just compiles the findings, their recommendations, and
 * any ready-made deterministic fix. No AI involved.
 */
export function buildFixesText(
  report: Report,
  url: string,
  findings: Finding[],
  impacts: Record<string, number> = {},
): string {
  const issues = priorityFixes(findings, impacts);
  const fixMap = fixesByFinding(report);
  const when = report.fetched_at ? new Date(report.fetched_at) : null;
  const headline = (report.scorecard as { headline_score?: number } | null | undefined)?.headline_score;

  const out: string[] = ["# Astova — AI readiness fixes", "", `URL: ${url}`];
  if (when && !Number.isNaN(when.getTime())) out.push(`Scanned: ${when.toLocaleString()}`);
  if (typeof report.overall_score === "number") out.push(`Overall score: ${report.overall_score}/100`);
  if (typeof headline === "number") out.push(`AI Retrievability: ${headline}/100`);
  out.push("");

  if (issues.length === 0) {
    out.push("No issues found — every deterministic check passed. ✓");
    return out.join("\n");
  }

  out.push(`## ${issues.length} ${issues.length === 1 ? "thing" : "things"} to fix`, "");
  issues.forEach((f, i) => {
    const pts = impacts[f.id];
    const tag = [sev(f.severity).label, pts ? `+${pts} pts` : null].filter(Boolean).join(" · ");
    out.push(`### ${i + 1}. ${f.title}${tag ? `  (${tag})` : ""}`);
    if (f.pillar) out.push(`- Area: ${f.pillar}`);
    if (f.evidence) out.push(`- What's wrong: ${f.evidence}`);
    if (f.recommendation) out.push(`- How to fix: ${f.recommendation}`);
    const fix = fixMap[f.id];
    if (fix?.content) {
      out.push(`- Ready-to-apply (${fix.language || fix.kind || "code"}):`);
      out.push("```" + (fix.language || ""), fix.content.trimEnd(), "```");
    }
    out.push("");
  });
  out.push("---", `Generated by Astova · ${url}`);
  return out.join("\n");
}
