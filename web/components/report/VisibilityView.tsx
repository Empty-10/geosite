...
import type { Rate, ShareOfVoiceRow, VisibilityReport } from "./visibilityTypes";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--ink)" }}>
      ...
      <main style={{ maxWidth: 920, margin: "0 auto", padding: "24px 20px 80px" }}>
        ...
        <p style={{ fontSize: 14, color: "var(--text-2)", marginBottom: 20, maxWidth: 660, lineHeight: 1.6 }}>
          Ask AI engines real questions and measure whether your brand appears and your domain gets{" "}
          <em>cited</em>. Sampled on a date, reported with a 95% confidence interval and sample size — never a bare
          number. Engine: Claude with web search.
        </p>
        ...
        <div style={{ marginBottom: 16 }}>
          <Field label="Competitors (optional, comma-separated — for share of voice)" value={competitors} onChange={setCompetitors} placeholder="Mixpanel, Amplitude, PostHog" />
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 26 }}>
          ...
          <span style={{ fontSize: 12, color: "var(--text-3)" }}>Each run queries the engine live — bounded and rate-limited.</span>
        </div>
...
function Results({ report }: { report: VisibilityReport }) {
  ...
  return (
    <div style={{ animation: "dmFade 0.3s ease both" }}>
      ...
      >
        <Chip>MEASURED</Chip>
        sampled {when.toLocaleString()} · n={report.sample_size}
        {report.requested !== report.sample_size ? ` of ${report.requested}` : ""} · {report.engine}
      </div>
      ...
      {report.share_of_voice.length > 1 && (
        <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {report.prompts.map((p, i) => (
          <div
            key={i}
            style={{ border: "1px solid var(--border)", borderRadius: 11, background: "var(--surface)", padding: "12px 14px" }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
              <Tag on={p.cited} label={p.cited ? "Cited" : "Not cited"} color={p.cited ? C.accent : C.text3} />
              <Tag on={p.appeared} label={p.appeared ? "Brand named" : "Not named"} color={p.appeared ? M : C.text3} />
              {p.found_not_cited && <Tag on label="Found, not cited" color={C.warn} />}
              {p.competitors.length > 0 && (
                <span style={{ fontSize: 11, color: "var(--text-3)" }}>vs {p.competitors.join(", ")}</span>
              )}
            </div>
            <div style={{ fontSize: 13.5, color: "var(--text)", marginBottom: p.excerpt ? 6 : 0 }}>{p.prompt}</div>
            {p.excerpt && <div style={{ fontSize: 12.5, color: "var(--text-3)", lineHeight: 1.5 }}>{p.excerpt}…</div>}
          </div>
        ))}
      </div>

      <p style={{ fontSize: 11.5, color: "var(--text-3)", marginTop: 18, lineHeight: 1.6 }}>
        Intervals are Wilson 95% bounds on a small sample — directional, not precise. Add more questions for tighter
        bounds. A different engine (or the same engine on another day) can give different results; that variance is
        why this is labeled MEASURED, not VERIFIED.
      </p>
    </div>
  );
}
  ...
  return (
    <div
      style={{
        flex: 1,
        minWidth: 240,
        border: `1px solid ${rgba(M, 0.3)}`,
        borderRadius: 14,
        background: rgba(M, 0.05),
        padding: "16px 18px",
      }}
    >
      ...
      <div style={{ fontSize: 12, color: "var(--text-3)", fontFamily: "var(--mono)" }}>
        95% CI {pct(rate.ci[0])}–{pct(rate.ci[1])}%
      </div>
      {/* CI band */}
      <div style={{ position: "relative", height: 5, borderRadius: 999, background: "var(--raised)", marginTop: 10 }}>
        <div
          style={{
            position: "absolute",
            left: `${pct(rate.ci[0])}%`,
            width: `${Math.max(2, pct(rate.ci[1]) - pct(rate.ci[0]))}%`,
            height: "100%",
            borderRadius: 999,
            background: rgba(M, 0.55),
          }}
        />
        <div
          style={{ position: "absolute", left: `${pct(rate.rate)}%`, top: -2, width: 2, height: 9, background: M }}
        />
      </div>
        ...
        <div key={r.name} style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span
            style={{
              width: 130,
              fontSize: 13,
              color: r.isTarget ? M : "var(--text-2)",
              fontWeight: r.isTarget ? 600 : 400,
              whiteSpace: "nowrap",
              overflow: "hidden",
          ...
          <div style={{ flex: 1, height: 8, borderRadius: 999, background: "var(--raised)", overflow: "hidden" }}>
            <div style={{ width: `${pct(r.share)}%`, height: "100%", background: r.isTarget ? M : "var(--border-strong)" }} />
          </div>
          <span style={{ width: 64, textAlign: "right", fontSize: 12.5, color: "var(--text-3)", fontFamily: "var(--mono)" }}>
            {pct(r.share)}% ({r.mentions})
          </span>
        </div>
      ))}
    </div>
  ...
  return (
    <span
      style={{
        ...
        color: M,
        border: `1px solid ${rgba(M, 0.4)}`,
        background: rgba(M, 0.1),
        padding: "2px 8px",
        borderRadius: 999,
        fontFamily: "var(--mono)",
      }}
  ...
  return (
    <span
      style={{
        ...
        border: `1px solid ${rgba(color, on ? 0.4 : 0.2)}`,
        background: on ? rgba(color, 0.1) : "transparent",
        padding: "1px 8px",
        borderRadius: 6,
      }}
    >
