// DELETE /api/monitors/:id  → stop monitoring a site. Thin proxy to the engine.
...
export async function DELETE(
  ...
  if (!/^\d+$/.test(id)) return Response.json({ error: "Invalid monitor id." }, { status: 400 });

  try {
    const res = await fetch(`${ENGINE_URL.replace(/\/$/, "")}/monitors/${id}`, {
      method: "DELETE",
