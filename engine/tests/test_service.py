...
def test_scan_returns_report_dict():
    ...
    with patch("astova_engine.service.scan", return_value=fake) as m:
...
def test_scan_surfaces_engine_error_as_200():
    ...
    with patch("astova_engine.service.scan", return_value=fake):
        r = client.post("/scan", json={"url": "x.test"})
    assert r.status_code == 200
...
def test_crawl_job_lifecycle():
    ...
    with patch("astova_engine.service.crawl", return_value=fake):
...
def test_crawl_caps_max_pages():
    ...
    with patch("astova_engine.service.crawl", side_effect=fake_crawl):
...
def test_cloudflare_logs_endpoint(monkeypatch):
    ...
    with patch("astova_engine.service.fetch_cloudflare_logs", side_effect=fake_fetch):
...
def test_scan_persists_and_diffs(monkeypatch, tmp_path):
    from astova_engine.models import Finding, Pillar, Severity, Status

    monkeypatch.setenv("ASTOVA_DB_PATH", str(tmp_path / "s.db"))
    ...
    with patch("astova_engine.service.scan", side_effect=[r1, r2]):
...
def test_performance_endpoint_returns_score_and_findings():
    with patch("astova_engine.service.fetch_pagespeed", return_value=_PSI):
...
def test_performance_endpoint_unavailable():
    with patch("astova_engine.service.fetch_pagespeed", return_value=None):
